...
func OpenDb() {
	var err error
	Db, err = leveldb.OpenFile("heroes.db", nil)
	if err != nil {
		log.Fatal(err)
	}
	//defer Db.Close()
	rows, err1 := Db.Get([]byte("heroes"), nil)
...
{ "id": 19, "name": "Magma" },
{ "id": 20, "name": "Tornado" }]`
	err = db.Put([]byte("heroes"), []byte(strs), nil)
	if err != nil {
		log.Fatal(err)
	}
...
func getHeroes(c *gin.Context) {
	rows, err := Db.Get([]byte("heroes"), nil)
	if err != nil {
		log.Fatal(err)
	}
	//defer rows.Close()
...
func StartGin() {
	...
	*/
	...
	router.POST("/heroes", func(c *gin.Context) {
		...
		log.Println(cMap)
		err = db.Put([]byte("heroes"), []byte(cMap["name"]), nil)
